...
export class KaspaWalletMobile extends KaspaWalletUI {
	...
	render() {
		...
		let donationAddresses = [
			["Devfund donations:", "kobra:qql0qdtl7q2kypx69lp72hc2fas46x22enqsfvfpw7g34l5uyz5264xjnfqdx"],
			["Mining address:", "kobra:qql0qdtl7q2kypx69lp72hc2fas46x22enqsfvfpw7g34l5uyz5264xjnfqdx"],
			//["KDX/WebWallet donations:", "kobra:qql0qdtl7q2kypx69lp72hc2fas46x22enqsfvfpw7g34l5uyz5264xjnfqdx"],
		]
			...
			<div class="flow-swipeable-row">
				...
				html`
						...
						<flow-expandable class="donation-info" no-info no-icon icon="-">
							...
							<p is="i18n-p">
								if you wish to further the development of the kobra ecosystem, we accept KODA donations at the following addresses:
							</p>
				...
				<div class="tab-content ${sCls('faucet')}" for="faucet">
					${this.faucetStatus ? this.faucetStatus : html`
						...
						<div class="faucet-ux">
							...
							<div class="margin">${i18nHTMLFormat('You have <b>[n] KODA</b> available.', KAS(this.faucetFundsAvailable || 0))}</div>
				...
				<div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
					<div class="info-ux">
						...
						<table>
							...
							<tr><td is="i18n-td">AMOUNT</td><td>${KAS(inUseUTXOs.satoshis || 0)} KODA</td></tr>
						</table>
						<flow-btn class="center-btn primary v-margin"
	...
	openAddressExplorer(address) {
		if (!address)
			return
		let url = `https://explorer.k0bradag.com/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatKAS(total) + ' KODA' : ''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatKAS(available)} KODA</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatKAS(pending)} KODA</span>
					</div>
				`}
            </div>
