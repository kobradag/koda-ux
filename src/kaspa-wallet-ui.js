	...
	renderTX({ hideTxBtn = false, onlyNonConfirmed = false } = {}) {
		...
		<div class="recent-transactions">
			...
			return html`<div class="tx-notification">
						${n.compoundUTXOs ?
					T(`Compounding UTXOs...`) :
					i18n.t(`Preparing transaction for [n] KODA ....`)
			...
			return html`
					<flow-expandable class="tx-row" static-icon expand ?txin=${tx.in} ?txmoved=${tx.isMoved} ?txout=${!tx.in}
						...
						<div class="tx-title" slot="title">
							...
							<div class="amount">
								${tx.in ? '' : '-'}${this.formatKAS(tx.amount)} KODA
							</div>
						</div>
						...
						<div class="tx-body">
							${tx.note}
							<div class="tx-id">
								<a target="_blank" href="https://explorer.k0bradag.com/txs/${tx.id}">${tx.id}</a>
							</div>
							<div class="tx-address">
								${tx.myAddress ? T('COMPOUNDING WALLET => ') : ''}
								<a target="_blank" href="https://explorer.k0bradag.com/addresses/${tx.address}">${tx.address}</a>
							</div>
						</div>
					</flow-expandable>
				`
		...
		return html`
			...
			return html`
					<div class="tx-row" ?txin=${tx.in} tx-version=${tx.version} ?txmoved=${tx.isMoved} ?txout=${!tx.in}>
						...
						<div class="tx-amount">${tx.in ? '' : '-'}${KAS(tx.amount)} KODA</div>
						<div class="br tx-note">${tx.note}</div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.k0bradag.com/txs/${tx.id.split(":")[0]}">${tx.id.split(":")[0]}</a>
						</div>
						<div class="tx-address">
							${tx.myAddress ? T('COMPOUNDING WALLET => ') : ''}
							<a target="_blank" href="https://explorer.k0bradag.com/addresses/${tx.address}">${tx.address}</a>
		...
		return html`
			...
			return html`
					<div class="tx-row" ?iscoinbase=${!tx.isCoinbase}>
						...
						<div class="tx-amount">${KAS(tx.satoshis)} KODA</div>
						<div class="br tx-mass"></div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.k0bradag.com/txs/${tx.txId}">${tx.id}</a>
						</div>
						<div class="tx-address">
							<a target="_blank" href="https://explorer.k0bradag.com/addresses/${tx.address}">${tx.address}</a>
						</div>
	...
	*/
	...
	requestFaucetFunds() {
		...
		showT9({
			...
			inputLabel: i18n.t('Amount in KODA')
		}, ({ value: amount, dialog }) => {
			let sompis = formatForMachine(amount || 0);
			if (sompis > this.faucetFundsAvailable) {
				let msg = i18n.t(`You can't request more than [n] KODA.`)
