...
class KaspaSendDialogMobile extends KaspaDialog {
	...
	renderBody({ estimating, estimateFee }) {
		return html`
			...
			<flow-input class="amount full-width" suffix-btn outer-border
				label="${T('Amount in KODA')}" @keyup=${this.onAmountChange}>
	...
	renderEstimate() {
		...
		<div class="estimate-tx">
			<table>
				...
				${dataFee ? html`<tr><td is="i18n-td">Data Fee</td><td>${KAS(dataFee)} KODA</td></tr>` : ''}
				${fee ? html`<tr><td is="i18n-td">Total Fee</td><td>${KAS(fee)} KODA</td></tr>` : ''}
				${totalAmount ? html`<tr><td is="i18n-td">Total Amount</td><td> ${KAS(totalAmount)} KODA</td></tr>` : ''}
			</table>
		</div>
		`
	}
	...
	showT9(e) {
		...
		showT9({
			value,
			heading: input.label.replace("in KODA", ""),
	...
	async sendAfterConfirming() {
		...
		if (estimate.fee > this.alertFeeAmount) {
			let msg = i18n.t('Transaction Fee ([n] KODA) is very large.');
